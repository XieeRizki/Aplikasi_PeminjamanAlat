        ...
        @auth
            ...
            @if(auth()->user()->level === 'admin')
                ...
                <a href="{{ route('users.index') }}" class="flex items-center px-5 py-3 {{ request()->routeIs('users.*') ? 'bg-slate-700 text-white border-l-4 border-emerald-500 font-semibold' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} transition-all duration-200">
                    ...
                    <span class="ml-3">Kelola Users</span>
                </a>
            ...
            @if(auth()->user()->level === 'petugas')
                ...
                <a href="{{ route('monitoring-pengembalian.index') }}" class="flex items-center px-5 py-3 {{ request()->routeIs('monitoring-pengembalian.*') ? 'bg-slate-700 text-white border-l-4 border-emerald-500 font-semibold' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} transition-all duration-200">
            ...
            @if(auth()->user()->level === 'peminjam')
                ...
                <a href="{{ route('alat.index') }}" class="flex items-center px-5 py-3 {{ request()->routeIs('alat.*') ? 'bg-slate-700 text-white border-l-4 border-emerald-500 font-semibold' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} transition-all duration-200">
                    <i class="fas fa-tools w-5"></i>
                    <span class="ml-3">Daftar Alat Tersedia</span>
                </a>
                ...
                <!-- Mengembalikan Alat -->
                <a href="{{ route('pengembalian-alat.index') }}" class="flex items-center px-5 py-3 {{ request()->routeIs('pengembalian-alat.*') ? 'bg-slate-700 text-white border-l-4 border-emerald-500 font-semibold' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} transition-all duration-200">
                    <i class="fas fa-undo w-5"></i>
                    <span class="ml-3">Kembalikan Alat</span>
                </a>
            @endif
        @endauth
    </nav>
